...
#ifndef qdeclarativeeditview_p_h
#define qdeclarativeeditview_p_h
...
class QDeclarativeEditSettings;
class QDeclarativeEditViewPrivate;
class QNetworkRequest;
class QDeclarativeEditView;
class QDeclarativeEditViewPrivate;

class QDeclarativeEditPage : public QWebPage {
    Q_OBJECT
public:
    explicit QDeclarativeEditPage(QDeclarativeEditView *parent);
    ~QDeclarativeEditPage();
public:
    ...
    QDeclarativeEditView *viewItem();
};

class GraphicsEditView : public QGraphicsWebView {
    Q_OBJECT
public:
    GraphicsEditView(QDeclarativeEditView* parent = 0);
protected:
...
private:
    QDeclarativeEditView *parent;
    QPointF pressPoint;
    QBasicTimer pressTimer;
    int pressTime; // milliseconds before the touch event becomes a "tap and hold"
    friend class QDeclarativeEditView;
};

class QDeclarativeEditViewAttached;

// TODO: browser plugins

class QDeclarativeEditView : public QDeclarativeItem {
    Q_OBJECT
    ...
    Q_PROPERTY(QDeclarativeEditSettings* settings READ settingsObject CONSTANT)
...
public:
    QDeclarativeEditView(QDeclarativeItem *parent = 0);
    ~QDeclarativeEditView();
    ...
    QDeclarativeEditSettings *settingsObject() const;

    bool renderingEnabled() const;
    void setRenderingEnabled(bool);

    QDeclarativeListProperty<QObject> javaScriptWindowObjects();
    //static QDeclarativeEditViewAttached* qmlAttachedProperties(QObject*);
    static QDeclarativeEditViewAttached* qmlAttachedProperties(QObject* o);
...
private Q_SLOTS:
    ...
    QDeclarativeEditView* createWindow(QWebPage::WebWindowType type);

private:
    void updateContentsSize();
    void init();
    virtual void componentComplete();
    Q_DISABLE_COPY(QDeclarativeEditView)
    QDeclarativeEditViewPrivate* d;
    ...
    friend class QDeclarativeEditPage;
};

class QDeclarativeEditViewAttached : public QObject {
    Q_OBJECT
    Q_PROPERTY(QString windowObjectName READ windowObjectName WRITE setWindowObjectName)
public:
    QDeclarativeEditViewAttached(QObject* parent)
        : QObject(parent)
    {
    }
...
class QDeclarativeEditSettings : public QObject {
    Q_OBJECT
...
QML_DECLARE_TYPE(QDeclarativeEditView)
QML_DECLARE_TYPE(QDeclarativeEditSettings)
QML_DECLARE_TYPEINFO(QDeclarativeEditView, QML_HAS_ATTACHED_PROPERTIES)
